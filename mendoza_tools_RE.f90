...
implicit none
contains
subroutine mendoza(bp,q,er,Prob,Zprob,Z,BB,kappa,    Fhh,Fll,gamma,beta,rho,sigmay,RR,kappah,kappal,alpha,uptd,bmin,bmax)
	
	use basic
	use tools
		
	! local 
	integer :: i,j,k, outfreq=5, iter_tol=7000, iter, pos
	integer, parameter ::  NS=9, NB=20, NK=2, NTOTAL=18 !!!!!!!! NS,NB, NK cannot be variables
	real	:: Kprob(2,2), tol = 1E-4, kappas(2), QQ, d2, val=0., inter(NTOTAL)=0., delta=1E-5, error_EE = 1.
	real    :: cm, cl=0.001, ch=2., gridStepSize
	real,  dimension(NB,NTOTAL):: c, EE, b, mup=0., emu=0., emuq=0., cbind, bpmax, oldc, oldq, oldbp
	real, dimension(NB,NTOTAL):: mupq=0., yq, yt, eyq=0., prueba(4,4), prueba2(2,2)=1.
...
!	integer, intent(inout) ::
	...
	Prob = kronecker(ones(NK,NK),NK,NK, Zprob,NS,NS)*kronecker(Kprob,2,2,ones(NS,NS),NS,NS)
	...
	do while (d2 > tol .and. iter_tol > iter )
		...
		do j = 1,NTOTAL		
			...
			do k = 1,NTOTAL
				inter(k) =  piecewise(BB,mup(:,k),NB,bp(i,j))
			end do	
			...
			!***** Piecewise interpolation by Johannes Brumm *****
			do k = 1,NTOTAL				
				inter(k) =  piecewise(BB,mupq(:,k),NB,bp(i,j))	
			end do
			...
			!***** Piecewise interpolation by Johannes Brumm *****
			do k = 1,NTOTAL
				inter(k) =  piecewise(BB,yq(:,k),NB,bp(i,j))			
			end do
    	...
    	if (iter==0) then
			...
			!call print_matrix(emu,NB,NTOTAL)
			!write(*,*) 'mup', iter	
			!call print_matrix(mup,NB,NTOTAL)
			!write(*,*) 'c', iter
			!call print_matrix(c,NB,NTOTAL)
			!stop
		end if
